...
namespace cnc
{
    ...
    f32 atan( f32 z) 
    {
        ...
        if (z > 1.0) {
            return CNC_PI/2 - cnc::atan(1.0 / z);
        } else if (z < -1.0) {
            return -CNC_PI/2 - cnc::atan(1.0 / z);
        } else {
    ...
    f32 lineangle( v2 a, v2 b )
    {
        f32 angle = cnc::atan2( (b.y - a.y), (b.x - a.x) );
    ...
    f32 length( v2  a ) 
    {
        f32 result = cnc::sqr( a.x * a.x + a.y * a.y ); 
        return result;
    }
    
    v2 normalize( v2 v ) 
    {
        f32 len = cnc::length( v );
        return (len > 0.0f) ? vec2( v.x / len, v.y / len) : vec2( 0.0f, 0.0f );
    }
    
    v2 reflect( v2 a, v2 n) 
    {
        v2 result;
    ...
    {
        m4 result = cnc::translationMatrix( position.x, position.y );
        return result;
    }
    
    m4 identityMatrix()
    {
        return cnc::translationMatrix( 0.0f, 0.0f );
    }
    ...
    bool vector_test()
    {
        ...
        bool add     = cnc::v2_equal( a+b,    vec2(  1.0f,  1.0f ) );
        bool sub     = cnc::v2_equal( c-a,    vec2(  9.0f, 10.0f ) );
        bool scale   = cnc::v2_equal( c*2.0f, vec2( 20.0f, 20.0f ) );
        bool dot     = (cnc::dot( a, b ) == 0.0f );
        bool length  = (cnc::length( vec2( 3.0f, 0.0f ) ) == 3.0f );
        bool norm    = cnc::length( cnc::normalize( c ) ) == 1.0f;
        bool reflect = cnc::v2_equal( cnc::reflect( d, a), a ); 
